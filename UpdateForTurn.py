...
def right_blue_sign(image):
    # hsv = cv2.cvtColor(image, cv2.COLOR_BGR2HSV) 
    lower_bluee = np.array([179,178,255]) 
    upper_bluee = np.array([179,178,255]) 
    ...
    blue_ratio1 = np.sum(mask1) / (mask1.size * 255)
    global detect_blue_right_sign_bool
    if blue_ratio > 0.00002:
        detect_blue_right_sign_bool = 1
        return res
    elif blue_ratio1 > 0.00002:
        detect_blue_right_sign_bool = 1
        return res1
    else:
        detect_blue_right_sign_bool = 0
...
if __name__ == "__main__":
    try:
        while True:
            ...
            #         t = 0
            if mode == 2:
                pidd = PIDD(Kp=1.8, Ki=0, Kd=0, setpoint=10)
